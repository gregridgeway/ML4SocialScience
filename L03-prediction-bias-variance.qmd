...
<!-- In terminal -->
<!-- quarto render L03-prediction-bias-variance.qmd -->
<!-- quarto render L03-prediction-bias-variance.qmd --cache-refresh  -->

<!-- git commit L03* -m "commit message" -->
<!-- git status -->
<!-- git push -->
...
for(i in 1:nrow(dTest))
{
  dTest$p[i] <- dTrain |>
    ...
    # use the (laplace corrected) average of these as the prediction
    summarize(p = (1 + sum(wave4dropout)) / (2 + k)) |>
    pull(p)
}
```

Note that I have used a Laplace correction when estimating computing $\hat p$.

$$
\hat p = \frac{1+\sum_{\mathbf{x}_i \in N_k(\mathbf{x})} y_i}{2+k}
$$
Adding 1 in the numerator and the number of outcome categories (2 in this case) to the denominator shrinks the probability estimates toward $\frac{1}{2}$. This is particularly important when an observations nearest neighbors are all 0s or all 1s. In such cases we do not *really* think that their predicted probabilities should be 0 or 1. The Laplace correction keeps the predicted probabilities from ever equaling 0 or 1.
...
#| cache: true
design <- data.frame(k = c(10*(1:9), 100*(1:9), 1000*(1:6), 6500 + 10*(0:70)),
                     misclassCost = NA)
for(iK in 1:nrow(design))
{
  ...
  for(i in 1:nrow(dTest))
  {
    ...
    #   mutate(distance = abs(ses - dTest$ses[i])) |>
    #   slice_min(distance, n=k) |>
    #   summarize(p = (sum(wave4dropout)+1)/(k+2)) |>
    #   pull(p)
    ...
    j <- abs(dTrain$ses - dTest$ses[i]) |>
      order() |> 
      head(k)
    dTest$p[i] <- (sum(dTrain$wave4dropout[j])+1)/(k+2)
  }
...
kBest <- design |> 
  slice_min(misclassCost, with_ties=FALSE) |>
  pull(k)
abline(v=kBest)
kBest
```
...
```{r}
#| label: check with FNN
#| echo: false
#| eval: false
k <- kBest
a <- FNN::knn(train = dTrain |> select(ses),
              test = dTest |> select(ses),
              cl = dTrain$wave4dropout,
              k=k,
              prob=TRUE)

dTest <- dTest |>
  mutate(pFNN = attributes(a)$prob,
         pFNN = ifelse(a=="1", pFNN, 1-pFNN),
         pFNN = (pFNN*k+1)/(k+2))
```

It appears that the best choice for $k$ is around 7000 Let's look at a narrower range around $k=7000$. If you ran the previous block of code, you realized that it can take a while to try the `r nrow(design)` values of $k$. It is a good habit to run such for loops in parallel. Each loop is independent of the other so we could have our computer work on multiple values of $k$ at the same time. The `doParallel` package makes this really easy. You can use `detectCores()` to learn how many cores you have available to you.
```{r}
#| message: false
library(doParallel)
```
...
design <- data.frame(k = seq(6800,7200,by=3), misclassCost = NA)
...
#| fig-cap: !expr paste0("Misclassification cost by choice of $k$ $(",min(design$k)," \\leq k\\leq ",max(design$k),")$")
...
```{r}
# extract the probability of majority class from knnPred
dGrid$p <- attributes(knnPred)$prob
# convert to P(spam)
dGrid$p <- ifelse(knnPred=="1", dGrid$p, 1-dGrid$p)
# and apply Laplace correction
dGrid$p <- (1+10*dGrid$p) / (2+10)
```

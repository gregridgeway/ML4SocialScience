...
Let's put all this to work on the our officer-involved shooting reports. We will define documents by their first 7 right singular vectors (columns of $\mathbf{V}$). R has a built-in `kmeans()` function in the default `stats` package that will do all the work for us.

First, we need to decide on how many clusters we should use. The code below explores the relationship between the number of clusters, $k$, and $R^2$. This is a key part of the "elbow method," a common strategy for selecting an appropriate number of clusters. By running k-means repeatedly with increasing values of $k$ (from 1 to 40 in this case), and recording the corresponding SSE, we can visualize how much the clustering improves as $k$ increases. A plot of SSE versus $k$ typically shows a steep drop initially and then levels off. The "elbow" point, where the rate of improvement sharply slows, is often a good choice for the number of clusters. It represents a balance between underfitting and overfitting the structure in the data.
```{r}
#| fig-cap: Relationship between $k$ and the within cluster sum of squares
#| label: fig-kvsse
...
Let's examine the top 10 terms for the larges three clusters.
```{r}
hovertext |> arrange(desc(size)) |> head(3)
```
Perhaps the first two are variations on the typical officer-involved shooting involving an offender, possibly armed (cluster \#5). Cluster \#4 clearly seems to be shootings of dogs, commonly pit pulls, near a residence.

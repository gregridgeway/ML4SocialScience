...
<!-- In terminal -->
<!-- quarto render L9-propensity-score-estimation.qmd -->
<!-- quarto render L9-propensity-score-estimation.qmd --cache-refresh  -->
...
# Neyman-Rubin-Holand causal model
...
Let's pursue our propensity score strategy by fitting a standard logistic regression model predicting prior use heroin from the four demographic variables of interest. We use `quasibinomial` here just so that R does not complain about non-integer weights.
...
The column $w$ contains out propensity score weights multiplied by the NSDUH sampling weights. We first set up a new survey design object with these new weights and proceed with a weighted regression model to compute the ATT.
...
Note that after weighting, the treatment and control groups look very similar. The largest difference in the table above is `{r} round(ps2$ks,3)`. You can this largest KS statistic from the `ps2` object.
```{r}
ps2$ks
```

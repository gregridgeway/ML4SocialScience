...
<!-- In terminal -->
<!-- quarto render L08-boosting.qmd -->
<!-- quarto render L08-boosting.qmd --cache-refresh  -->
...
plot(lasso1, xvar="norm")
maxL1 <- lasso1$beta |> t() |> tail(1) |> abs() |> sum()
...
ridge1 <- glmnet(X[,-1], y, 
                 ...
                 lambda=seq(1000,0,length.out=1000))
plot(ridge1, xvar="norm")
maxL1 <- ridge1$beta |> t() |> tail(1) |> abs() |> sum()
points(rep(maxL1, length(betaTrue)-1), coef(lm1)[-1], 
       col=1:(length(betaTrue)-1),
       pch=15)
```

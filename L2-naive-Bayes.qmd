...
We will demonstrate the naive Bayes classifier using data on Georgia parolees. These data were used as part of the National Institute of Justice's [recidivism prediction challenge](https://data.ojp.usdoj.gov/Courts/NIJ-s-Recidivism-Challenge-Full-Dataset/ynf5-u8nk). The recidivism outcome, `Recidivism_Within_3years` takes the value `true` if the parolee is arrested for a new felony or misdemeanor crime within three years of the supervision start date.
...
wGender
# 3. convert to log odds, log of 2nd row/1st row
wGender <- log(wGender[2,]/wGender[1,])
wGender
```
Knowing that parolee is female parolee decreases the evidence in favor of reoffending, while knowing a parolee is male slightly increases the evidence for reoffending.
...
modNB |> 
  ...
  row_spec(1:nrow(modNB), color=colTab)
```
...
predNBwoe
```
Note that after the `left_join()` we have the correct weights of evidence the associate feature and its value. To make a prediction we just need to add all of the weights of evidence for each `ID` plus the prior weight of evidence.
...
kbl(tab,
    ...
    digits=0) |>
  row_spec(0, color="black", background = "green") |>
  row_spec(1:maxRows, color=colTab) |>
  row_spec(maxRows+1:3,background="green") |>
  kable_classic()
```
...
datRecid |> 
  filter(p > 0.5) |> 
  group_by(Training_Sample) |>
  summarize(falsePos=mean(Recidivism_Within_3years=="false"))

# false negative
datRecid |> 
  filter(p < 0.5) |> 
  group_by(Training_Sample) |>
  summarize(falsePos=mean(Recidivism_Within_3years=="true"))
```

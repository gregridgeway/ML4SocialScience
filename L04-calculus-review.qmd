...
3.  Find $\hat t$ that optimizes $f(t)=100+\frac{800t}{t^2+90000}$. Is it a maximum or a minimum?
...
The gradient "points" in the direction of the steepest _increase_ in $J$. So $-\nabla J$ is the direction in which we need to move our current choice of $\beta$ to optimize $J(\beta)$. That is, if we have a current guess for $\hat\beta$, then we can find an improved guess (one that makes $J(\beta)$ even smaller), by adjusting our guess as
$$
\begin{split}

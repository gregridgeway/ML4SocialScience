...
2.  Examples
    ...
    a.  $\mathbf{x}$=characteristics of an adolescent entering drug treatment $y$=passes a drug test 12 months after admission (a 0/1 outcome)

    b.  $\mathbf{x}$=characteristics of a firearm sale (purchaser, dealer, gun features) $y$=time when the firearm is recovered in connection to a crime (a positive, possibly censored, continuous outcome)

    c.  $\mathbf{x}$=customer history of book purchases $y$=title of the next book they will purchase (a very large multiclass outcome)

    d.  $\mathbf{x}$=demographics, ZIP code $y$=will vote in next election

    e.  $\mathbf{x}$=all previous words in a conversation $y$=next word produced in the conversation
...
abline(lm1, lwd=3)
```
This particular lines y-intercept and slope depend on the specific collection of 100 simulated pairs $(x_i, y_i)$. There are many lines that we *could* have gotten from 100 randomly generated $x$s and $y$s. Let's see what kind of lines we *could* have observed. I'll generate 50 datasets and draw the OLS line for each of them in grey.
```{r}
#| label: fig-var2
...
fx0.5$se.fit^2
```
The variance estimated from that single sample is rather close to to the true value we got from simulation. Variance is the one component that we can actually get a handle on even if we do not know the true $f(x)$.
...
We are going to put this all together now. In the end we are interested in the expected prediciton error, the difference between the actual value, $y$, and our predicted value, $\hat f\left(\frac{1}{2}\right)$. Formally,
$$
...
# Predicting high school drop out using the National Education Longitudinal Study of 1988 (NELS88)
...
I have already acquired and cleaned up the dataset. If you want to do further work with the NELS data you may want to look at the `NELS data prep.R` script. We will start by loading some necesary R packages and the cleaned up dataset.
...
head(nels0, 3)
```
We will treat the first 5,000 cases students as training data. This is also generally not a good idea since observations next to each other often have some connection (same school, same interviewer, similar dates). I am doing this here, again, for simplicity of the exercise.
```{r}
dTrain <- nels0 |> head(5000)
dTest  <- nels0 |> tail(nrow(nels0)-5000)
```
...
dTest |>
  summarize(misclass = 19*mean(wave4dropout==1))
...
We were rather presumptuous in the previous section to set $k=1000$. Perhaps there is another value of $k$ that could do even better. Let's try a range of values for $k$ and measure their misclassification costs. I first make a `design` data frame that lists the values of $k$ that I want to try. It will take to long to try *every* choice for $k$ between 1 and 5000, so I created a selection of 36 values for $k$ in that range. This will allow us to narrow in on the best choice for $k$.

```{r}
#| label: codeSearchOptimalK
#| cache: true
design <- data.frame(k = c(1:10, 10*(2:10), 100*(2:10), 500*(3:10)),
                     misclassCost = NA)
...
Let's have a look at what this the optimal knn classifier look like.
```{r}
...
Here is a famous example dataset from the late 1990s. This is one of the early efforts to create an email spam filter.
...
-   `word_xxx`: percentage of words in the email that are xxx
...
Load of `ggplot2` so we can make some interesting figures.
```{r}
#| message: false
...
```
Already from @fig-plotDollarExclamation you can see some separation between spam and non-spam emails. Spam have a strange number of dollar signs and exclamation marks and non-spam emails tend to have none.
...
knnPred <- FNN::knn(
  ...
  prob    = TRUE)
```
A few important things to note about predicted probabilities from `knn()`. First, the returned value in `knnPred` will be a vector of factor values "0" and "1". It returns the majority class of the $k$ nearest neighbors, ignoring misclassification costs. Second, the predicted probabilities are attached to `knnPred` as an attribute, so we use `attributes(knnPred)$prob` to extract them. Finally, the values stored in `attributes(knnPred)$prob` are *not* the probabilities of being spam, but rather the probability of being from the majority class. If the majority class is a 0, that the values in `attributes(knnPred)$prob` are actually $1-P(\mathrm{spam})$. Very tricky! Be careful!
...
```{r}
#| label: fig-10nnGrid
#| fig-cap: Predicted probability based on percentage of $ and !
#| fig-width: 6
#| fig-height: 5
ggplot() +
  ...
  scale_y_continuous(trans='sqrt')
```
Note that there is a a bit of a "scaling" issue. The percentage of characters that are dollar signs ranges from 0% to 6% while the percentage of characters that are exclamation marks ranges from 0% to 30%. Therefore, emails tend to be closer to each other in terms of dollar signs than they are in terms of exclamation marks just because the difference in spread. Let's regenerate the figure, but this time I am going to divide all the `char_freq_$` values by their maximum (6) and all the `char_freq_!` by their maximum (30) so that the new versions both range from 0 to 1.



```{r}
#| label: fig-10nnGridReScale
#| fig-cap: Predicted probability based on percentage of $ and !, rescaled to be between 0 and 1
#| fig-width: 6
#| fig-height: 5
dTrain <- dTrain |> 
...
ggplot() +
  ...
  scale_y_continuous(trans='sqrt')
```
The classification boundaries have changed just by changing how we measure distance between emails. This is one of the fundamental challenges of using the knn classifier. The choice of distance measure will change the predicted probablities, the classification boundaries, and ultimately the predictive performance of the model.
...
Also you can see that how we transform the email features has an effect on the misclassification rate. Taking the tenth root of the each of the features and standardizing seems to do the best. Why tenth root? I have no idea. Almost certainly there is another way to transform these email features to get even better predictive performance.

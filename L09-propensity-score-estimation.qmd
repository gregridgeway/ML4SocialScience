...
```{r}
lm(y~treat+x1+x2+x1:x2, data=dataCor) |>
  summary()
```
...
Before we move on to a different simulation, note that this dataset had a strong correlation between $\mathbf{x}$ and $t$. At the same time, $t$ was strongly associated with $y$.
...
dataInd <- data.frame(treat=rep(0:1,each=100)) |>
  mutate(x1 = rbinom(200, 1, 0.6),
         x2 = rbinom(200, 1, 0.3*x1 + 0.9*(1-x1)),
         y  = rnorm(200, cbind(1,x1,x2,x1*x2) %*% beta, 1))
```
...
Since these weights make the control cases resemble the treatment cases, our treatment effect estimate is called the *average treatment effect on the treated* (ATT). The ATT compares what actually happened to the treatment group to what would have happened had they not been treated. In the majority of social science inquiries, this is the primary quantity of interest.
...
\mathbb{E}(Y(0)|t=1) &= \iint y_0p(y_0,\mathbf{x}|t=1)\; d\mathbf{x}\,dy_0 \\
 ...
 &\approx \frac{p(t=0)}{p(t=1)}
    \frac{\sum_{i=1}^n (1-t_i)y_i  
        ...
        {\sum_{i=1}^n (1-t_i)} \\
 &\approx \frac{\sum_{i=1}^n (1-t_i)  
         \frac{p(t=1|\mathbf{x}_i)}{1-p(t=1|\mathbf{x}_i)}y_i}
        {\sum_{i=1}^n (1-t_i)\frac{p(t=1|\mathbf{x}_i)}{1-p(t=1|\mathbf{x}_i)}} \\
 &= \frac{\sum_{t_i=0} w_iy_i}
         {\sum_{t_i=0} w_i} \\
\end{split}
$$

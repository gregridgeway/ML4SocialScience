...
```{r}
#| echo: false
model <- load_model_hdf5("lstm_nextcharV2.keras")
```
...
Here's another version with two LSTM layers that has a lot more parameters, but this one will take about 24 hours on a laptop. I might try this someday.
```{r}
#| eval: false
# 20 hours
# model <- keras_model_sequential() |>
#   layer_embedding(input_dim = length(chars)+1, output_dim = 40) |>
...
```{r}
#| eval: false
model$save("lstm_nextcharV2.keras")
```

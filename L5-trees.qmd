...
library(kableExtra)
set.seed(20010618)
colTab <- ifelse(knitr::is_html_output(), "white", "black")
```
...
kbl(dAge, col.names = c("Ratio of D-aspartic to L-aspartic acid","Age")) |> 
  kable_classic() |>
  row_spec(0, color="black", background = "green") |>
  row_spec(1:nrow(dAge), color=colTab)
```
</div>
...
: Choosing split points {#tab-ratioDLacidSplit1}
...
: Refractive index, sodium concentration, and window glass (sorted by refractive index) {#tab-glassSortRI}
</div>
...
: Refractive index, sodium concentration, and window glass (sorted by sodium concentration)  {#tab-glassSortNA}
</div>
...
Here is the best two-split classification tree. I first separates the three non-window glass with high sodium. Then among the low sodium concentration samples, it peels off the four windows with low refractive index.
```{r}
...
When the dataset is small, LOOCV is feasible, but it quickly becomes too much computatinal effort for large datasets. For large datasets, the most common approach is to use 10-fold cross validation. Rather than holding out 1 observation at a time, 10-fold cross-validation holds out 10\% of the data at a time and uses the remaining 90\% of the data for learning the tree.
...
#| fig-cap: Cross-validated error of glass classification using all available feature by complexity parameter and tree size
...
And let's see how the tree has has carved up the 2D space.
...
Note this is a similar tree as the one we saw earlier. This time around we included family income and used 10-fold cross-validation to arrive at the optimal tree.
...
Here we learn something potentially interesting. For low SES there is a high dropout risk, but that risk seems to be reduced if the family structure includes mom, mom and dad, or an other relative (not dad alone, not mom and step-dad, not dad and step-mom).

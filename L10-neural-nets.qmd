...
col1and2transparent <- 
  ...
  paste0("10")
# col1and2transparent <- c("#FF69B410","#00FF7F10")
```
...
We start with some guess for $\hat\beta$, perhaps $(0,0,0)$. Then once we pass our data through the neural network we get predicted values and we learn how badly we did by computing $\sum (y_i-\hat y_i)^2$. Now we can improve our guess for $\hat\beta$ by computing the gradient of $J$ with respect to $\beta$. We will chain rule our way to figure out the gradient.
...
The network in @fig-nnhidenlayer1 two continuous inputs, passes them through a hidden layer of four nodes, which passes their output to a final output node.

```{tikz nnhiddenlayer1}
#| label: fig-nnhidenlayer1
#| echo: false
...
Returning to the network in @fig-nnhidenlayer1, the hidden nodes marked with a $[1]$ superscript in them all have the form
$$
\mathbf{a}^{[1]} = 
\begin{bmatrix}
...
$$
Now that we have the algorithm for updating the output layer parameters, we can move backwards through the network to update the next layer's parameters. Here we look at layer $[1]$ and adjust the weight on input $k$ associated with hidden node $j$.
$$
\begin{split}
    \frac{\partial J}{\partial w_{jk}^{[1]}} &= \sum_{i=1}^n  
     \frac{\partial J}{\partial \hat y_i}
     \frac{\partial\hat y_i}{\partial a_{ij}^{[1]}} 
     \frac{\partial a_{ij}^{[1]}}{\partial w_{jk}^{[1]}} \\
     &= -\sum_{i=1}^n \frac{y_i-\hat y_i}{\hat y_i(1-\hat y_i)}
        \hat y_i (1-\hat y_i)w_{j}^{[2]}
...
We will use the algorithm we developed earlier to fit a "single-layer perceptron" model, like the one shown in @fig-linearnn.
```{r}
...
# A more complex decision boundary in two dimensions
...
To remedy the problem with the linear neural network, we are going to fit the model shown in @fig-nnhidenlayer1, with a hidden layer and using the sigmoid activation function. I borrowed this code structure from this blog post on [Building A Neural Net from Scratch Using R](https://rviews.rstudio.com/2020/07/20/shallow-neural-net-from-scratch-using-r-part-1/). The code on the website has several bugs that have been fixed here.
...
For the most serious deep learning analysis, that work is done directly in Tensorflow. So, if you really want to learn this area well, start with Keras and then start digging into workign with Tensorflow (or PyTorch).

As I developed these notes, Tensorflow and Keras do not work with Python 3.12 (at present the latest version). That will certainly change. You may wish to start by working with Python 3.11.
...
library(keras)
install_keras(python_version="3.11") # doesn't work with 3.12 yet
```

...
4.  Find $\hat p$ that optimizes $\ell(p)=6\log(p)+10\log(1-p)$. Is it a maximum of minimum?
...
\end{split}
$$
There are plenty of examples where Newton's method does not work well (slow to converge) or at all (diverges or converges to a local maximium/minimum). For example, if we started with $p_0=0$ or $p_0=1$, then for this problem $\hat p$ would get stuck at 0 or 1 and never move to a better value. Also note that if we started at $p_0=\frac{3}{8}$ then it also does not change, but that is because $\frac{3}{8}$ is the right answer!
